...
export function GuidedTour() {
    ...
    useEffect(() => {
        ...
        if (element) {
            ...
            // Add scroll listener
            const handleScrollOrResize = () => updatePosition(element);
            ...
            if (currentStep.selector.includes('vault') && !path.includes('vaults')) {
                // Not on vaults page yet, wait
                return;
            }
            
            // Element might render later, try a few times
            let retries = 0;
            const interval = setInterval(() => {
                ...
                if (el) {
                    ...
                    clearInterval(interval);
                    if (path === '/' && currentStep.selector.includes('vault')) {
                        // Ignore
                    } else {
    ...
    }, [isActive, currentStepIndex, updatePosition, handleNext]);
